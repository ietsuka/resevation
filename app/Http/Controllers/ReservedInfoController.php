...
class ReservedInfoController extends Controller
{
  ...
  public function csv(Request $request)
  {
    $response = new StreamedResponse(function () use ($request) {
      $date = $request->input('date');
      $start = date("Y/m", strtotime($date));
      $start = $start . "/01";
      $end = date("Y/m/d", strtotime($start . '+1 month'));


      $reserved = ReservedInfo::whereBetween('reserved_date', [$start, $end])->get();
      $fpath = './' . date("Ymd", strtotime($date)) . '.csv';
      echo CreateCsv::create_csv($reserved, $fpath);
    });
